...
public class Enemy {
	...
	public Enemy(Problem p, Drawable asteroid, Drawable explosion, int X,
			int Y, int Velocity, int KillBound, SpaceInvadersThread t) {
		x = X;
		y = Y;
	...
	public void Draw(Canvas c, int ViewWidth, int ViewHeight) {
		if (!Killed) {
			...
			c.drawText(problemStr, x, y + height + (textBound.height() / 2),
					mPaint);
